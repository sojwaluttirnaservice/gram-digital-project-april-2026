document.addEventListener('DOMContentLoaded', function () {
	...
	dropdownButtons.forEach((button) => {
		...
		function setZIndexRecursively(menu, level) {
			...
			const nestedButtons = menu.querySelectorAll(
	...
	mobileMenuBtn.addEventListener('click', function (e) {
		...
		const mobileMenuHamIcon = document.getElementById(
			'mobile-menu-ham-icon'
		);
		...
		if (isMenuHidden) {
			...
			// Force repaint to make sure transition starts from max-h-0 and opacity-0
			...
			// Wait for the transition to finish before applying display: none
			setTimeout(() => {
