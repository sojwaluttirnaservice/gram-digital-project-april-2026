$(document).ready(function () {
  ...
  // modal visibility related MAYBE
  ...
  // HERE 9999 means its too mcuh value, assuming initially
  // not changint this as of now, since its old code -> commented on 30 March 2026
   ...
   *
   * LATEST: check file paymentForOptions.js -> commented on 30 March 2026
   */
  ...
  // THIS FUNCTION IS OPENING THE MODAL AND ALSO SETTING THE VALUES IN THE MODAL
  function openPaymentTypeModel() {
    ...
    $("#pay-mobile-no").val(
      $("#feu_mobile_no").val() === "-" ? "" : $("#feu_mobile_no").val(),
    );
  }
  ...
  let totalPreviewDataPreviousPaniTax =
    +$("#previewDataLastSpacialWaterTax").val() +
    +$("#previewDataLastGenealWaterTax").val();
  ...
  $(document).on("click", ".button-offline-pay", async function (e) {
    ...
    let isSamanya = paymentDetails.paymentFor == 1;
    try {
      ...
      // let values = getSamanyaNewVasuliAndUpdateFormNineData();

      let sendData = {
        paymentData: {
            ...paymentDetails, 
            tax_category: paymentDetails.paymentFor == 1 ? "SAMANYA" : "PANI"
        },
        ...(paymentDetails.paymentFor == 1
          ? getSamanyaNewVasuliAndUpdateFormNineData()
          : {}),
        ...(paymentDetails.paymentFor == 2
          ? getWaterNewVasuliAndUpdateFormNineData()
          : {}),
      };

      let endpoint = isSamanya ? "/tax-payer/samanya" : "/tax-payer/water";
      ...
      const callback = isSamanya
        ? // samanya
          function () {
            window.open(
              "/print/tax-payer/samanya?t_no=" +
                data.insertIdNewTaxVasuli +
                "&i=" +
                $("#userId").val() +
                "&txn=" +
                $("#transaction_number").val(),
            ...
            // window.history.go(-2);
            console.log("Updating data in Samanya tax");
            if (
              srcPage &&
      ...
      if (success) {
        alertjs.success(
          {
            t: "भरणा झाला",
            m:  message || `${isSamanya ? "सामान्य" : "पाणी"} कर भरला गेला.`,
          },
          () => {
            callback();
          },
        ...
        alertjs.warning({
          t: "भरणा झाला नाही",
          m: message || `${isSamanya ? "सामान्य" : "पाणी"} कर भरला गेला नाही.`,
        });
      }
    } catch (err) {
      ...
      alertjs.warning({
        t: "भरणा झाला नाही",
        m:  err?.message || `${isSamanya ? "सामान्य" : "पाणी"} कर भरला गेला नाही.`,
      });
    }

    // replacign this whole below code into new code
    /** 
    $.ajax({
      url: "/payment/save",
      method: "POST",
  ...
  $(document).on("click", ".button-online-pay", async function (e) {
    e.preventDefault();
    let isSamanya = paymentDetails.paymentFor == 1;
    try {
      ...
      formData.set("paymentData", JSON.stringify(sendData.paymentData));
      formData.set("newVasuliData", JSON.stringify(sendData.newVasuliData));
      formData.set("updateFormNine", JSON.stringify(sendData.updateFormNine));
      ...
      error: function (error) {
        ...
        alertjs.warning({
          ...
          m:
            ...
            "Payment not done, please try again later",
        });
      },
    });
    */
  ...
  /**
   * Collects new tax payment data from the form and calculates updated values for Form Nine.
   *
   * This function reads values from the HTML form fields, converts them to numbers where necessary,
   * and creates two objects:
   *
   * 1. `newVasuliData` - Represents the new tax payment entry to be saved in another table.
   * 2. `updateFormNine` - Represents the updated values for the original Form Nine record
   *    after subtracting the new payments.
   *
   * It also reformats the bharnaDate from DD/MM/YYYY to YYYY-MM-DD.
   *
   * @typedef {Object} newVasuliData
   * @property {number} lastBuildingTax
   * @property {number} currentBuildingTax
   * @property {number} lastDivaTax
   * @property {number} currentDivaTax
   * @property {number} lastArogyaTax
   * @property {number} currentArogyaTax
   * @property {number} lastTaxFine
   * @property {number} lastTaxRelief
   * @property {number} lastCleaningTax
   * @property {number} currentCleaningTax
   * @property {number} lastFireblegateTax
   * @property {number} currentFireblegateTax
   * @property {number} lastTreeTax
   * @property {number} currentTreeTax
   * @property {number} lastEducationTax
   * @property {number} currentEducationTax
   * @property {number} formNineId
   * @property {number} userId
   * @property {string} bharnaDate
   * @property {string} amountInWords
   * @property {string} checkNo
   * @property {number} finalTax
   *
   * @typedef {Object} updateFormNine
   * @property {number} currentArogyaTax
   * @property {number} currentBuildingTax
   * @property {number} currentDivaTax
   * @property {number} currentCleaningTax
   ...
   *
   * @function
   * @returns {{ newVasuliData: newVasuliData, updateFormNine: updateFormNine }}
   */
  function getSamanyaNewVasuliAndUpdateFormNineData() {
    ...
    var updateFormNine = {
      currentArogyaTax:
        details["currentArogyaTax"] - newVasuliData["currentArogyaTax"],
      currentBuildingTax:
        details["currentBuildingTax"] - newVasuliData["currentBuildingTax"],
      currentDivaTax:
        details["currentDivaTax"] - newVasuliData["currentDivaTax"],
      currentCleaningTax:
        details["currentCleaningTax"] - newVasuliData["currentCleaningTax"], // fixed typo
      currentEducationTax:
        details["currentEducationTax"] - newVasuliData["currentEducationTax"],
      currentFireblegateTax:
        details["currentFireblegateTax"] -
        newVasuliData["currentFireblegateTax"],
      currentTreeTax:
        details["currentTreeTax"] - newVasuliData["currentTreeTax"],
      lastArogyaTax: details["lastArogyaTax"] - newVasuliData["lastArogyaTax"],
      lastBuildingTax:
        details["lastBuildingTax"] - newVasuliData["lastBuildingTax"],
      ...
      lastCleaningTax:
        details["lastCleaningTax"] - newVasuliData["lastCleaningTax"],
      lastEducationTax:
        details["lastEducationTax"] - newVasuliData["lastEducationTax"],
      lastFireblegateTax:
        details["lastFireblegateTax"] - newVasuliData["lastFireblegateTax"],
    ...
    return { newVasuliData, updateFormNine };
  }

  /**
   * Collects new water tax payment data from the form and calculates updated values for Form Nine.
   *
   * This function reads values from the HTML form fields, converts them to numbers where necessary,
   * and creates two objects:
   *
   * 1. `newVasuliData` - Represents the new water tax payment entry to be saved in another table.
   * 2. `updateFormNine` - Represents the updated values for the original Form Nine record
   *    after subtracting the new payments.
   *
   * It also reformats the bharnaDate from YYYY-MM-DD to DD-MM-YYYY if present.
   *
   * @typedef {Object} newVasuliData
   * @property {number} lastSpacialWaterTax
   * @property {number} currentSpacialWaterTax
   * @property {number} lastGenealWaterTax
   * @property {number} currentGenealWaterTax
   * @property {number} formNineId
   * @property {number} userId
   * @property {string} checkNo
   * @property {number|string} finalWaterTax
   * @property {string} bharnaDate - Date in DD-MM-YYYY format
   *
   * @typedef {Object} updateFormNine
   * @property {number} lastSpacialWaterTax
   * @property {number} currentSpacialWaterTax
   ...
   * @returns {{ newVasuliData: newVasuliData, updateFormNine: updateFormNine }}
   */
  function getWaterNewVasuliAndUpdateFormNineData() {
    /** @type {newVasuliData} */
    var newVasuliData = {
    ...
    var updateFormNine = {
      ...
      currentSpacialWaterTax:
        details["currentSpacialWaterTax"] -
        newVasuliData["currentSpacialWaterTax"],
      lastGenealWaterTax:
        details["lastGenealWaterTax"] - newVasuliData["lastGenealWaterTax"],
      currentGenealWaterTax:
        details["currentGenealWaterTax"] -
        newVasuliData["currentGenealWaterTax"],
      formNineId: Number($("#formNineId").val()),
      userId: Number($("#userId").val()),
      checkNo: $("#checkNo").val(),
    };
  ...
  function updateSamanyaTaxIntoDB(paymentId = "") {
    ...
    var updateFormNine = {
      ...
      currentCleaningTax:
        details["currentCleaningTax"] -
        newVasuliData["currentCurrentCleaningTax"],
      currentEducationTax:
        details["currentEducationTax"] - newVasuliData["currentEducationTax"],
      currentFireblegateTax:
        details["currentFireblegateTax"] -
  ...
  $(document).on("click", "#annual-report-samanya-print-btn", function (e) {
    ...
    window.open(
      `/print/tax-payer/all-samanya?startYear=${startYear}&endYear=${endYear}`,
      "_blank",
    );
  });
  ...
  $(document).on("click", "#monthly-report-samanya-print-btn", function (e) {
    ...
    window.open(
      `/print/tax-payer/all-samanya?month=${month}&year=${year}`,
      "_blank",
    );
  });
  ...
  $(document).on("click", "#annual-report-pani-print-btn", function (e) {
    ...
    window.open(
      `/print/tax-payer/all-pani?startYear=${startYear}&endYear=${endYear}`,
      "_blank",
    );
  });
  ...
  $(document).on("click", "#monthly-report-pani-print-btn", function (e) {
    ...
    window.open(
      `/print/tax-payer/all-pani?month=${month}&year=${year}`,
      "_blank",
    );
  });
