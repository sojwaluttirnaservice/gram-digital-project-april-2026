$(document).ready(function () {
    $(document).on('click', '#show-namuna-31-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/31/report';
    ...
    const handleDeleteNamuna31Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/31/delete';
    ...
    $(document).on('click', '.delete-namuna-31-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna31Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna31Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-31-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/31/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/31/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
