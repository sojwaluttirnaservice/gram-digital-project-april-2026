$(document).ready(function () {
    $(document).on('click', '#show-namuna-12-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/12/report';
    ...
    const handleDeleteNamuna12Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/12/delete';
    ...
    $(document).on('click', '.delete-namuna-12-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna12Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna12Entry(id);
            }
        });
    });

    $(document).on('click', '#print-namuna-12-btn', function (e) {
        e.preventDefault();
        ...
        window.open(`/namuna/12/print?month=${month}&year=${year}`, '_blank');
    });
});
