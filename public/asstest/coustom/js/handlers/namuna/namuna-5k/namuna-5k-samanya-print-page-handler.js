document.addEventListener("DOMContentLoaded", function () {
  ...
  const fetchLastDateOfNamuna5k = async () => {
    try {
      const response = await fetch("/namuna/5k/samanya/date");
  ...
  // handle Save Payment
  const handleSaveOfflinePaymentInNamuna5kSamanya = async (n5kData) => {
    try {
      const { success, message, data } = await fetch("/namuna/5k/samanya/pay", {
        method: "POST",
        body: n5kData,
      }).then((r) => r.json());
  ...
  const button = document.getElementById(
    "save-save-offline-payment-namuna-5k-samanya-btn",
  );
  ...
  if (button) {
    button.addEventListener("click", function (e) {
      ...
      handleSaveOfflinePaymentInNamuna5kSamanya(n5kData);
    });
  }
});
