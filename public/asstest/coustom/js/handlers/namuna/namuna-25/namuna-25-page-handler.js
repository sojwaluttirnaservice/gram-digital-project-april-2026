$(document).ready(function () {
    $(document).on('click', '#show-namuna-25-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/25/report';
    ...
    const handleDeleteNamuna25Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/25/delete';
    ...
    $(document).on('click', '.delete-namuna-25-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna25Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna25Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-25-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/25/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/25/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
