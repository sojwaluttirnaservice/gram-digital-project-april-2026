$(document).ready(function () {
    ...
    $('#submit-namuna-24-entry').on('click', async function (e) {
        e.preventDefault();
        ...
        try {
            // Validate the form
            if (!$('#namuna-24-form').valid()) {
                alertjs.warning({ t: 'WARNING', m: 'कृपया सर्व आवश्यक फील्ड्स भरा' });
                submitButton.prop('disabled', false);
                return;
            }

            const form = $('#namuna-24-form')[0];
            let namuna24FormData = new FormData(form);

            // Format date fields before sending the data
            const transaction_date = namuna24FormData.get('transaction_date');
            const certificate_date = namuna24FormData.get('certificate_date');
            const resolution_date = namuna24FormData.get('resolution_date');
            const authority_order_date = namuna24FormData.get('authority_order_date');

            // Reformat dates before setting them back into FormData
            namuna24FormData.set('transaction_date', formatDate(transaction_date));
            namuna24FormData.set('certificate_date', formatDate(certificate_date));
            namuna24FormData.set('resolution_date', formatDate(resolution_date));
            namuna24FormData.set('authority_order_date', formatDate(authority_order_date));

            // Send POST request for saving
            const response = await fetch('/namuna/24/save', {
                method: 'POST',
                body: namuna24FormData,
            });
    ...
    $('#update-namuna-24-entry').on('click', async function (e) {
        e.preventDefault();
        ...
        try {
            ...
            const response = await fetch(`/namuna/24/update`, {
