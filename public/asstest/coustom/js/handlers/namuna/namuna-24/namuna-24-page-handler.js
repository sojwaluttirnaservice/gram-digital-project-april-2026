$(document).ready(function () {
    $(document).on('click', '#show-namuna-24-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/24/report';
    ...
    const handleDeleteNamuna24Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/24/delete';
    ...
    $(document).on('click', '.delete-namuna-24-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna24Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna24Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-24-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/24/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/24/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
