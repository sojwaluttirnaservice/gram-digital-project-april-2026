$(document).ready(function () {
    $(document).on('click', '#show-namuna-23-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/23/report';
    ...
    const handleDeleteNamuna23Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/23/delete';
    ...
    $(document).on('click', '.delete-namuna-23-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna23Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna23Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-23-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/23/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/23/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
