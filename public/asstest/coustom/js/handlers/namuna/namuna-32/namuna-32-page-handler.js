$(document).ready(function () {
    $(document).on('click', '#show-namuna-32-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/32/report';
    ...
    const handleDeleteNamuna32Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/32/delete';
    ...
    $(document).on('click', '.delete-namuna-32-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna32Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna32Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-32-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/32/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/32/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
