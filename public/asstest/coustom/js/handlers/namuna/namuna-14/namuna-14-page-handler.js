$(document).ready(function () {
    $(document).on('click', '#show-namuna-14-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/14/report';
    ...
    const handleDeleteNamuna14Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/14/delete';
    ...
    $(document).on('click', '.delete-namuna-14-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna14Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna14Entry(id);
            }
        });
    });

    $(document).on('click', '#print-namuna-14-btn', function (e) {
        e.preventDefault();
        ...
        window.open(`/namuna/14/print?month=${month}&year=${year}`, '_blank');
    });
});
