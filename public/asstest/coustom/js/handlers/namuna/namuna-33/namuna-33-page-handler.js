$(document).ready(function () {
    $(document).on('click', '#show-namuna-33-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/33/report';
    ...
    const handleDeleteNamuna33Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/33/delete';
    ...
    $(document).on('click', '.delete-namuna-33-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna33Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna33Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-33-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/33/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/33/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
