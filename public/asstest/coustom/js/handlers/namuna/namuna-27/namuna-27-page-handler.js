$(document).ready(function () {
    $(document).on('click', '#show-namuna-27-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/27/report';
    ...
    const handleDeleteNamuna27Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/27/delete';
    ...
    $(document).on('click', '.delete-namuna-27-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna27Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna27Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-27-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/27/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/27/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
