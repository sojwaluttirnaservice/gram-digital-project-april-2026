$(document).ready(function () {
    $(document).on('click', '#show-namuna-30-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/30/report';
    ...
    const handleDeleteNamuna30Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/30/delete';
    ...
    $(document).on('click', '.delete-namuna-30-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna30Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna30Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-30-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/30/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/30/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
