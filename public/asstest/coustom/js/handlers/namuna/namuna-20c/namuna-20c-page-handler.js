$(document).ready(function () {
    $(document).on('click', '#show-namuna-20c-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/20/c/report';
    ...
    const handleDeleteNamuna20CEntry = async (id) => {
        try {
            const deleteUrl = '/namuna/20/c/delete';
    ...
    $(document).on('click', '.delete-namuna-20c-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna20CEntry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna20CEntry(id);
            }
        });
    });
