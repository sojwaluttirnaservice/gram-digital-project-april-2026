$(document).ready(function () {
    $(document).on('click', '#show-namuna-29-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/29/report';
    ...
    const handleDeleteNamuna29Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/29/delete';
    ...
    $(document).on('click', '.delete-namuna-29-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna29Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna29Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-29-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/29/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/29/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
