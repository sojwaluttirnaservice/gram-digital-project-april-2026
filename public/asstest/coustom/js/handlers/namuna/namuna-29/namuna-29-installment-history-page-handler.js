$(document).ready(function () {
    ...
    const handleDeleteNamuna29InstallmentEntry = async (id) => {
        try {
            const deleteUrl = '/namuna/29/installment/delete';
    ...
    $(document).on('click', '.delete-namuna-29-installment-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna29InstallmentEntry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna29InstallmentEntry(id);
            }
        });
    });
