$(document).ready(function () {
    $(document).on('click', '#show-namuna-11-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/11/report';
    ...
    const handleDeleteNamuna11Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/11/delete';
    ...
    $(document).on('click', '.delete-namuna-11-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna11Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna11Entry(id);
            }
        });
    });

    $(document).on('click', '#print-namuna-11-btn', function (e) {
        e.preventDefault();
        ...
        window.open(`/namuna/11/print?month=${month}&year=${year}`, '_blank');
    });
});
