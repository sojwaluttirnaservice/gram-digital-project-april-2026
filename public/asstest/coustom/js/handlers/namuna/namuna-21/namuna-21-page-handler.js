$(document).ready(function () {
    $(document).on('click', '#show-namuna-21-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/21/report';
    ...
    const handleDeleteNamuna21Entry = async (id) => {
        try {
            const deleteUrl = '/namuna/21/delete';
    ...
    $(document).on('click', '.delete-namuna-21-entry-btn', function (e) {
        e.preventDefault();

        const id = $(this).attr('data-namuna21Entry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna21Entry(id);
            }
        });
    });
    ...
    $(document).on('click', '#print-namuna-21-btn', function (e) {
        e.preventDefault();
        ...
        if (printType === 'month-year') {
            ...
            window.open(`/namuna/21/print?month=${month}&year=${year}`, '_blank');
        } else {
            ...
            window.open(`/namuna/21/print?fromYear=${fromYear}&toYear=${toYear}`, '_blank');
        }
    });
});
