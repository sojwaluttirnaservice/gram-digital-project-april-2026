$(document).ready(function () {
    $(document).on('click', '#show-namuna-20-report-btn', function (e) {
        e.preventDefault();
        const optionValue = +$('[name="show-report-by"]').val();
        const reportBaseUrl = '/namuna/20/report';
    ...
    const handleDeleteNamuna1OtherExpendituresEntry = async (id) => {
        try {
            const deleteUrl = '/namuna/1/expenditure/other-expenditures/delete';
    ...
    $(document).on('click', '.delete-namuna-1-other-expenditure-entry-btn', function (e) {
        e.preventDefault();

        const id = +$(this).attr('data-namuna1OtherExpenditureEntry');

        alertjs.deleteSpl('Confirm Delete?', (status) => {
            if (status) {
                handleDeleteNamuna1OtherExpendituresEntry(id);
            }
        });
    });

    $(document).on('click', '#print-namuna-20-btn', function (e) {
        e.preventDefault();
        ...
        window.open(`/namuna/20/print?month=${month}&year=${year}`, '_blank');
    });
});
