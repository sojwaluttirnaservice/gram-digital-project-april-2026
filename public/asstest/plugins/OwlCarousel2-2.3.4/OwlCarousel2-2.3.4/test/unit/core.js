...
function before_and_after_remove(options) {
  ...
  equal(
    simple.html(),
    one.html(),
    "Inner HTML before and after remove one equals.",
  );
  ...
  equal(
    simple.html(),
    two.html(),
    "Inner HTML before and after remove two equals.",
  );
  ...
  equal(
    simple.html(),
    all.html(),
    "Inner HTML before and after remove all equals.",
  );
}
...
function before_and_after_remove_add(options) {
  ...
  equal(
    simple.html(),
    simpleClone.html(),
    "Inner HTML before and after `remove()` and `add()` equals.",
  );
}
...
test("invalidate", function () {
  ...
  deepEqual(
    carousel.invalidate(),
    ["first"],
    "One invalid part after invalidating one.",
  );
  ...
  deepEqual(
    carousel.invalidate(),
    ["first", "second"],
    "Two invalid parts after invalidating a second one.",
  );
  ...
  deepEqual(
    carousel.invalidate(),
    ["first", "second"],
    "Two invalid parts after invalidating a part twice.",
  );
  ...
  deepEqual(
    carousel.invalidate("first"),
    ["first"],
    "Invalidating one part returns it directly.",
  );
});
...
test("destroy", function () {
  ...
  equal(
    simple.get(0).outerHTML.replace(/\s{2,}/g, ""),
    expected,
    "Outer HTML before create and after destroy is equal.",
  );
});
