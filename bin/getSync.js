...
const backfillGpDastavegList = async () => {
	const transaction = await ps_gram_panchayet.sequelize.transaction();
	try {
		console.log(
			'🔄 Backfilling gp_dastavegList for all gram panchayets...'
		);

		const gps = await ps_gram_panchayet.findAll({ transaction });

		const targetDastaveg = [
			'बक्षिस पत्राने ',
			'खरेदीखत',
			'सोळ हक्काने ',
			'वाटणी पत्रानुसार ',
			'मृत्यू पत्रानुसार ',
			'हक्कसोळ रजिस्टर प्रमाणे ',
			'लिगल कोर्ट आदेश वारस हक्काने ',
			'वारस हक्काने '
		];

		for (const gp of gps) {
			let currentList = [];
			if (gp.gp_dastavegList) {
				try {
					currentList = JSON.parse(gp.gp_dastavegList);
		...
		await transaction.commit();
		console.log('✅ gp_dastavegList backfill complete');
	} catch (error) {
		await transaction.rollback();
		console.error('❌ gp_dastavegList backfill failed:', error);
	}
};
