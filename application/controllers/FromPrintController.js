...
let FromPrintController = {
	...
	printFormEightSinglePlain: asyncHandler(async (req, res) => {
		...
		const [userData] = await FormEightModel.formEightUser(
			res.pool,
			queryData
		);
...
*/
	...
	printpPageForm8KarNihayYadiAndOther: asyncHandler(async (req, res) => {
		...
		let queryRes =
			await FormPrintModel.getDistinctMalmattaCountExcludingOblique(
				res.pool
			);
	...
	form8Pratidnyapatra: function (req, res, next) {
		...
		ZPModel.getZpDetails(res.pool)
			.then((result) => {
				zpData = result[0];
				return FormEightModel.getTotalPrintFormEightUser(
					res.pool,
					y1,
					y2
				);
			...
			.then((total) => {
				...
				return FormEightModel.printFormEightUserLimit(
					res.pool,
					y1,
					y2,
					tp,
			...
			.then((result) => {
				if (result.length == 0) {
					...
					userData = result;
					return FormEightModel.printGetFromEightTaxTotalData(
						res.pool
					);
				}
			})
			.then((result) => {
				if (result != 999) {
					totalArray = result;
					return FormEightModel.printGetFromEightTaxSampleData(
						res.pool
					);
				} else {
					return 999;
	...
	printFormNineAllSamanya: asyncHandler(async (req, res) => {
		...
		const data = await FormNineModel.printFormNineAllSamanya(
			res.pool,
			y1,
			y2
		);
	...
	printFormEightAll: asyncHandler(async (req, res) => {
		...
		const [{ totalUsers: totalRecords }] =
			await FormEightModel.getForm8UserCount(res.pool, { property_type });
		const userData = await FormEightModel.printFormEightUserLimit(
			res.pool,
			y1,
			y2,
			tp,
		...
		const result2 = await FormEightModel.printGetFromEightTaxTotalData(
			res.pool
		);
		if (result2 != 999) {
			totalArray = result2;
			const result3 = await FormEightModel.printGetFromEightTaxSampleData(
				res.pool
			);
			if (result3 != 999) {
	...
	printFormEightAllImage: asyncHandler(async (req, res) => {
		...
		const totalArray = await FormEightModel.printGetFromEightTaxTotalData(
			res.pool
		);

		const totalTaxSample =
			await FormEightModel.printGetFromEightTaxSampleData(res.pool);
    ...
    */
	...
	cardPrint: asyncHandler(async (req, res) => {
		...
		let userData = await FormEightModel.printFormEightUserLimit(
			res.pool,
			y1,
			y2,
			tp,
		...
		let [{ totalUsers }] = await FormEightModel.getForm8UserCount(
			res.pool,
			{ property_type }
		);
	...
	printFromNineSamanya: asyncHandler(async (req, res) => {
		let { year1, year2 } = req.params;
		let data = await FormNineModel.getSamanyaPrintData(
			res.pool,
			year1,
			year2
		);
	...
	printFromNineSamanyaOther: asyncHandler(async (req, res) => {
		let year = req.params;
		const data = await FormNineModel.getSamanyaPrintData(
			res.pool,
			year.year1,
			year.year2
		);
	...
	printFromNineSamanyaKar: asyncHandler(async (req, res) => {
		let year = req.params;
		let data = await FormNineModel.getSamanyaPrintDataNew(
			res.pool,
			year.year1,
			year.year2
		);
		...
		ZPModel.getZpDetails(res.pool)
			...
			.then((result) => {
				...
				for (let i = 0; i < result.length; i++) {
					...
					taxes.buildingTax.lastTotalBuildingTax +=
		...
		ZPModel.getZpDetails(res.pool)
			...
			.then((result) => {
				for (let i = 0; i < result.length; i++) {
					...
					taxes.generalWaterTax.lastGenealWaterTax +=
						+data.lastGenealWaterTax;
					taxes.generalWaterTax.currentGenealWaterTax +=
						+data.currentGenealWaterTax;

					taxes.specialWaterTax.lastSpacialWaterTax +=
						+data.lastSpacialWaterTax;
					taxes.specialWaterTax.currentSpacialWaterTax +=
	...
	printPaniChaluKarMagniDemand: function (req, res, next) {
		...
		ZPModel.getZpDetails(res.pool)
			...
			.then((result) => {
				...
				result.forEach((data) => {
					dataObject.generalWaterTax.currentGeneralWaterTax +=
						data.currentGenealWaterTax;
		...
		ZPModel.getZpDetails(res.pool)
			...
			.then((result) => {
				...
				for (let i = 0; i < pages; i++) {
					...
					dataObject.malmattaNoTo =
						array1[array1.length - 1].feu_malmattaNo;
			...
			.then((result) => {
				...
				for (let i = 0; i < pages; i++) {
					...
					array1.forEach((data, i) => {
						dataObject.lastSpecialWaterTax +=
							data.lastSpacialWaterTax;
						dataObject.currentSpecialWaterTax +=
							data.currentSpacialWaterTax;
						dataObject.totalSpecialWaterTax +=
							data.totalSpacialWaterTax;

						dataObject.lastCommonWaterTax +=
							data.lastGenealWaterTax;
						dataObject.currentCommonWaterTax +=
							data.currentGenealWaterTax;
						dataObject.totalCommonWaterTax +=
	...
	printMagniBill: function (req, res, next) {
		...
		ZPModel.getZpDetails(res.pool)
			...
			.then((total) => {
				...
				return FormNineModel.getSamanyaPrintData(
					res.pool,
					y1,
					y2,
					p,
	...
	bankQRCodeMagniBill: async function (req, res, next) {
		try {
			...
			const total = await FormNineModel.getTotalPrintCount(
				res.pool,
				y1,
				y2
			);
			...
			const result = await FormNineModel.getSamanyaPrintData(
				res.pool,
				y1,
				y2,
				p,
	...
	printMeterPaniNondaniBill: function (req, res, next) {
		...
		ZPModel.getZpDetails(res.pool)
			.then((result) => {
				zpData = result[0];
				return FormPrintModel.getAllUserMeterPrintDetails(
					res.pool,
					usageFrom,
					usageTo
				);
	...
	taxPayerSamanyaPrint: asyncHandler(async (req, res) => {
		...
		if (isNaN(Number(form8UserId))) {
			return sendApiResponse(
				...
				400,
				false,
				"'i' must be a valid number"
			);
		}

		const formResult = await FormPrintModel.formEightUser(res.pool, {
			id: form8UserId
		});
		if (formResult.length === 0) {
			return res
				.status(404)
				.send({ message: 'Form 8 user Details Not Found' });
		}
		...
		if (taxResult.length === 0) {
			return res
				.status(404)
				.send({ message: 'Taxpayer Samanya Details Not Found' });
		}
	...
	taxPayerSamanyaPrintMonthOrYearWise: async (req, res) => {
		try {
			...
			if (month && year) {
				...
				receipts = await FormPrintModel.getMonthwiseTaxPayerSamanya(
					res.pool,
					{
						month: monthNum,
						year: yearNum
			...
			// fetch all records, by default not provding any year as of now
				...
				receipts = await FormPrintModel.getAllTaxPayerSamanya(
					res.pool,
					{
						startYear: start,
						endYear: end
	...
	taxPayerPaniPrintMonthOrYearWise: async (req, res) => {
		try {
			...
			if (month && year) {
				...
				receipts = await FormPrintModel.getMonthwiseTaxPayerPani(
					res.pool,
					{
						month: monthNum,
						year: yearNum
	...
	gpAhavalKarVasul: function (req, res, next) {
		...
		let mainYear = 0;
		mainYear = FromPrintController.calcMonth(
			currentMonth,
			month,
			currentYear
		);
		...
		ZPModel.getZpDetails(res.pool)
			...
			.then((result) => {
				...
				if ((l_month < 4 && currentMonth >= 4) || month == 4) {
					...
					return TaxPayerModel.getTotalValusliKarDetailsSamanya(
						res.pool,
						_y1,
						_y2
					);
			...
			.then((result) => {
				...
				if ((l_month < 4 && currentMonth >= 4) || month == 4) {
					...
					return TaxPayerModel.getTotalValusliKarDetailsPani(
						res.pool,
						_y1,
						_y2
					);
			...
			.then((result) => {
				...
				return TaxPayerModel.getValusliKarDetailsPani(
					res.pool,
					Number(month),
					mainYear
				);
			...
			.then((result) => {
				...
				formNineDetails.lastBuildingTax +=
					totalSamanyaVasuliOfYear.tpl_lastBuildingTax;
				formNineDetails.currentBuildingTax +=
					totalSamanyaVasuliOfYear.tpl_currentBuildingTax;
				formNineDetails.totalBuildingTax +=
					totalSamanyaVasuliOfYear.tpl_totalBuildingTax;
				formNineDetails.lastDivaTax +=
					totalSamanyaVasuliOfYear.tpl_lastDivaTax;
				formNineDetails.currentDivaTax +=
					totalSamanyaVasuliOfYear.tpl_currentDivaTax;
				formNineDetails.totalDivaTax +=
					totalSamanyaVasuliOfYear.tpl_totalDivaTax;
				formNineDetails.lastArogyaTax +=
					totalSamanyaVasuliOfYear.tpl_lastArogyaTax;
				formNineDetails.currentArogyaTax +=
					totalSamanyaVasuliOfYear.tpl_currentArogyaTax;
				formNineDetails.totalArogyaTax +=
					totalSamanyaVasuliOfYear.tpl_totalArogyaTax;
				formNineDetails.lastTaxFine +=
					totalSamanyaVasuliOfYear.tpl_lastTaxFine;
				formNineDetails.lastTaxFine += formNineDetails.lastYearTaxFine;
				formNineDetails.lastTaxRelief +=
					totalSamanyaVasuliOfYear.tpl_lastTaxRelief;
				formNineDetails.totalTax +=
					totalSamanyaVasuliOfYear.tpl_totalTax;
				...
				formNineDetails.lastGenealWaterTax +=
					totalPaniVasuliOfYear.tpl_lastGenealWaterTax;
			...
			.then((paymentDetails) => {
				...
				switch (payDetails.payment_for) {
					...
					case 2:
						...
						break;
					case 3:
						payDetails.certificate_name =
							'विवाह नोंदणी प्रमाणपत्र पेमेंट';
						break;
					case 4:
					...
					case 5:
						payDetails.certificate_name =
							'नमुना ८ प्रमाणपत्र पेमेंट';
						break;
					case 6:
						payDetails.certificate_name =
							'थकबाकी / निराधार प्रमाणपत्र पेमेंट';
						break;
					default:
						payDetails.certificate_name = 'प्रमाणपत्र पेमेंट';
				}
				payDetails.payment_date = myDate.changeDateFormat(
					payDetails.payment_date
				);
