...
const zpSchoolPointsController = {
    // View: List Page
    renderListPage: asyncHandler(async (req, res) => {
        const points = await zpSchoolPointsModel.getAll(res.pool);
        renderPage(res, 'user/gp/zp-school-points/list-page.pug', { points });
    }),

    // View: Add Page
    renderAddPage: asyncHandler(async (req, res) => {
        renderPage(res, 'user/gp/zp-school-points/add-page.pug');
    }),

    // View: Edit Page
    renderEditPage: asyncHandler(async (req, res) => {
        const { zpSchoolPointId: pointId } = req.params;

        if (!pointId || isNaN(pointId)) {
            return sendApiError(res, 400, false, 'अवैध ID.');
        }

        const [point] = await zpSchoolPointsModel.getById(res.pool, pointId);

        if (!point) {
            return sendApiError(res, 404, false, 'नोंद सापडली नाही.');
        }

        renderPage(res, 'user/gp/zp-school-points/edit-page.pug', { point });
    }),
    ...
    save: asyncHandler(async (req, res) => {
        ...
        const result = await zpSchoolPointsModel.add(res.pool, { point_desc });
    ...
    update: asyncHandler(async (req, res) => {
        ...
        const result = await zpSchoolPointsModel.update(res.pool, { id, point_desc });
    ...
    delete: asyncHandler(async (req, res) => {
        ...
        const result = await zpSchoolPointsModel.delete(res.pool, id);
...
module.exports = zpSchoolPointsController;