...
var indexController = {
  postNewStudent: function (req, res, next) {
    ...
    IndexModel.addNewStudent(res.pool, data)
      .then(function (res_data) {
        return IndexModel.updateStudentDetails(
          res.pool,
          res_data.insertId,
          data.mobileNumber,
        );
  ...
  getExamHomePage: function (req, res, next) {
    ...
    res.render("index", {
      title: "Exam Login",
      isSessionMessage:
        typeof req.session.SessionMessage === "undefined"
          ? false
          : req.session.SessionMessage,
      exam_link:
        typeof req.session.ExamLink === "undefined"
          ? false
  ...
  addStudentView: function (req, res, next) {
    ...
    var session_value =
      typeof req.session.SessionMessage == "undifined"
        ? false
        : req.session.SessionMessage;
          ...
          IndexModel.checkExamCreatedNot(res.pool, test_credensials)
            .then(function (data) {
              if (data.length == 1) {
                ...
                return IndexModel.createStudentQuestionPaper(
                  res.pool,
                  test_credensials,
                );
              }
  ...
  viewWarmUp: function (req, res, next) {
    ...
    var is_new_session_value =
      typeof req.session.isNew == "undifined" ? false : req.session.isNew;
    ...
    req.getConnection(function (err, connection) {
      if (err) {
        ...
        IndexModel.updateStudentTestTimerAndStatus(connection, updateDetails)
          .then(function (isStudentStatus) {
            return resultStatus.checkResultUpdated(
              isStudentStatus,
          ...
          .then(function (isQuestionPaperUpdated) {
            if (details.list.length > 0) {
              return IndexModel.updateTestQuestionDetails(
                connection,
                details.list[0],
              );
            } else {
