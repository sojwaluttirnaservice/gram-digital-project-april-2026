...
const pptController = {
	...
	update: asyncHandler(async (req, res) => {
		...
		let remaining_pages_header_image =
			existing.remaining_pages_header_image;
		...
		if (coverImage) {
			...
			await saveFile(
				coverImage,
				`${UPLOAD_PATHS.ppt.cover}/${cover_image}`
			);

			if (existing.cover_image) {
				await deleteFile(
					`${UPLOAD_PATHS.ppt.cover}/${existing.cover_image}`
				);
			}
		}
		...
		if (page1Image) {
			...
			await saveFile(
				page1Image,
				`${UPLOAD_PATHS.ppt.pages}/${page_1_image}`
			);

			if (existing.page_1_image) {
				await deleteFile(
					`${UPLOAD_PATHS.ppt.pages}/${existing.page_1_image}`
				);
			}
		}
		...
		if (page2Image) {
			...
			await saveFile(
				page2Image,
				`${UPLOAD_PATHS.ppt.pages}/${page_2_image}`
			);

			if (existing.page_2_image) {
				await deleteFile(
					`${UPLOAD_PATHS.ppt.pages}/${existing.page_2_image}`
				);
			}
		}
		...
		if (page3Image) {
			...
			await saveFile(
				page3Image,
				`${UPLOAD_PATHS.ppt.pages}/${page_3_image}`
			);

			if (existing.page_3_image) {
				await deleteFile(
					`${UPLOAD_PATHS.ppt.pages}/${existing.page_3_image}`
				);
			}
		}
	...
	delete: asyncHandler(async (req, res) => {
		...
		if (existingPpt.cover_image) {
			await deleteFile(
				`${UPLOAD_PATHS.ppt.cover}/${existingPpt.cover_image}`
			);
		}
