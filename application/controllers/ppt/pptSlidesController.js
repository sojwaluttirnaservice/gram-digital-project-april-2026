...
const pptSlidesController = {
	...
	createSlide: asyncHandler(async (req, res) => {
		...
		const beforeSubtitles = parseBodyArray(
			'before_image_subtitles',
			req.body
		);
		const afterTitles = parseBodyArray('after_image_titles', req.body);
		const afterSubtitles = parseBodyArray(
			'after_image_subtitles',
			req.body
		);
	...
	updateSlide: asyncHandler(async (req, res) => {
		...
		retainedBefore = retainedBefore.map((img) => {
			return {
				...
				image_subtitle:
					req.body[`existing_before_subtitle_${img.id}`] || ''
			};
		});
		...
		retainedAfter = retainedAfter.map((img) => {
			return {
				...
				image_subtitle:
					req.body[`existing_after_subtitle_${img.id}`] || ''
			};
		});
		...
		if (req.files) {
			...
			if (req.files.new_before_images) {
				const newBefore = Array.isArray(req.files.new_before_images)
					...
					: [req.files.new_before_images];
				const availableBeforeSlots = Math.max(
					0,
					2 - retainedBefore.length
				);
			...
			if (req.files.new_after_images) {
				const newAfter = Array.isArray(req.files.new_after_images)
					...
					: [req.files.new_after_images];
				const availableAfterSlots = Math.max(
					0,
					2 - retainedAfter.length
				);
		...
		const updatedSlide = await pptSlidesModel.updateSlide(
			res.pool,
			slideData
		);
	...
	deleteSlide: asyncHandler(async (req, res) => {
		...
		try {
			...
			console.error(
				'Failed to parse existing slide images during delete',
				e
			);
		}
	...
	getSlides: asyncHandler(async (req, res) => {
		...
		let populationHtml = '';
		if (ppt && ppt.is_main_ppt == 1) {
			const populationData =
				await villageController.getPopulationInfoData(res.pool);
			populationHtml = await new Promise((resolve) => {
				req.app.render(
					'user/ppt/population-slide-content.pug',
					{
						...populationData,
						t: res.locals.t,
					...
					(err, html) => {
						if (err) {
							console.error(
								'Error rendering population slide content:',
								err
							);
							resolve('');
						} else {
