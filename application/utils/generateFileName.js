...
const generateUniqueFileName = (file, prefix = '') => {
    ...
    if (!file || !file.name) throw new Error('Valid file object with a name is required');
    ...
    const numSeq = Math.floor(Math.random() * Math.pow(10, digitCount)).toString().padStart(digitCount, '0');
    ...
    else extension = path.extname(file.name).toLowerCase(); // fallback
    // Combine all parts
    return `${prefix}${charSeq}${numSeq}-${randomNum}-${timestamp}${extension}`;
};
