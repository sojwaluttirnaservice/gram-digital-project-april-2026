const gpCostRevaluationModel = {
    ...
    update: (pool, updateData) => {
        return new Promise((resolve, reject) => {
            const q = `
            UPDATE ps_n_3_gp_cost_revaluation
            SET
                year = ?,
                data_list = ?
            WHERE id = ?;
    ...
    getByYear: (pool, year) => {
        return new Promise((resolve, reject) => {
            ...
            SELECT 
                * 
            FROM ps_n_3_gp_cost_revaluation
            WHERE year = ?;
            `;
...
module.exports = gpCostRevaluationModel;
