...
module.exports = {
	getQuickUserInfoByAadhar: function (pool, data) {
		return new Promise((resolve, reject) => {
			var query = `SELECT formName,formMobile,formEmail,formAddress,formAadhar  FROM  ps_user_application WHERE formAadhar = ? AND is_deleted = 0 LIMIT 1`;
	...
	getNewApplicationList: function (pool) {
		return new Promise((resolve, reject) => {
			var query = `SELECT 
                   ...
                   FROM  ps_user_application WHERE documentVerifyDone = 0 AND is_deleted = 0`;
	...
	getRejectedApplicationList: function (pool) {
		return new Promise((resolve, reject) => {
			var query = `SELECT 
                    ...
                    DATE_FORMAT(docRemarkDate,"%d-%m-%Y") asdocRemarkDate,
                    DATE_FORMAT(create_date,"%d-%m-%Y") create_date
                  FROM  ps_user_application WHERE docRemark = 0 AND is_deleted = 0 ORDER BY docRemarkDate DESC,id DESC`;
			pool.query(query, (error, result) => {
				if (error) {
					reject(error);
				} else {
	...
	getAcceptedApplicationList: function (pool) {
		return new Promise((resolve, reject) => {
			var query = `SELECT 
                    ...
                    DATE_FORMAT(docRemarkDate,"%d-%m-%Y") asdocRemarkDate,
                    DATE_FORMAT(create_date,"%d-%m-%Y") create_date
                FROM  ps_user_application WHERE docRemark = 1 AND is_deleted = 0 ORDER BY docRemarkDate DESC,id DESC`;
			pool.query(query, (error, result) => {
				if (error) {
					reject(error);
				} else {
